...
In this Hands On you will build an interactive map of the US using
D3. I have adapted this tutorial from the [book]() by [name], which is the
best book on D3 I have found.
...
structures.  Scott put a copy in the gitrepo for his book. You can download
`us-states.json` from
...
both complex and nasty to work with by hand. Fortunately D3 comes with with
...
these with a _projection_. A is a mapping term referring to how real world
geographical data is draw onto 2D surface. D3 supports many different
projections. For this project we will use the [Albers equal-area conic
projection](http://en.wikipedia.org/wiki/Albers_projection), which is one of the
most common projections used for north america.  Add this code:
...
Scott's github site, download the us-ag-productivity-2004.csv file. This is a
...
color of the shape too...? Hmm. How should we convert data
...
Now the color can be calculated from the input value. Again, create a reusable
...
![Map of US States with color](img.png).screenshot
...
have everything else in place. Add another selectAll function chain with
...
the fill back to the original color on mouseout using the calcColor function
...
As an exercise to the reader, you can give each state a nice black border
with just a single line of code.


That's it! You just built an interactive map of the entire United States that will
run in almost any webbrowser. From here you could use different colors, plug in different
data, or combine it with other Javascript libraries.
